...
<?php
	...
	//jika tombol tambah admin ditekan
	if(isset($_POST['add'])){
        if(tambahadmin($_POST) > 0){
            ...
            echo"     
				<script>
					Swal.fire({
						title: 'Gagal',
						text: 'Admin tidak berhasil ditambahkan',
						icon: 'error'
					}).then(() => {
						window.location.href = 'users.php';
					});
	...
	if(isset($_POST['cetak'])){
		header("Location:pdf/userspdf.php") ;
	}
    ...
    if(isset($_GET['cari'])){
        ...
        $total_data = count(tampil("SELECT * FROM users WHERE username LIKE '%$cari%' ")) ;
        ...
        //tampilkan data
        $users = tampil("SELECT * FROM users WHERE username LIKE '%$cari%' LIMIT $dataawal,$dataperhalaman ") ; 
    }

    //jika user tidak mencari data
    else{
        $total_data = count(tampil("SELECT * FROM users")) ;
        $dataperhalaman = 5 ;
        $totalhalaman = ceil($total_data / $dataperhalaman) ;
        if(isset($_GET['hal'])){
            $halaktif = $_GET['hal'] ;
        ...
        $dataawal = ($halaktif * $dataperhalaman) - $dataperhalaman ; 
        $users = tampil("SELECT * FROM users LIMIT $dataawal,$dataperhalaman ") ;
    }
					   			...
					   			<div class="table-title">
					     			<div class="row">
						     			<div class="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
							    			<h2 class="ml-lg-2">Kelola Data Users</h2>
							 			</div>
