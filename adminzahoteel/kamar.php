...
<?php
    ...
    if(isset($_GET['cari'])){
        ...
        $total_data = count(tampil("SELECT * FROM kamar WHERE tipe LIKE '%$cari%' ")) ;
        ...
        //tampilkan data
        $kamar = tampil("SELECT * FROM kamar WHERE tipe LIKE '%$cari%' LIMIT $dataawal,$dataperhalaman ") ; 
    }

    //jika user tidak mencari data
    else{
        $total_data = count(tampil("SELECT * FROM kamar")) ;
        $dataperhalaman = 5 ;
        $totalhalaman = ceil($total_data / $dataperhalaman) ;
        if(isset($_GET['hal'])){
            $halaktif = $_GET['hal'] ;
        ...
        $dataawal = ($halaktif * $dataperhalaman) - $dataperhalaman ; 
        $kamar = tampil("SELECT * FROM kamar LIMIT $dataawal,$dataperhalaman ") ;
    }
											...
											<!----edit-modal start--------->
												<div class="modal fade" tabindex="-1" id="editEmployeeModal<?= $h['id'] ; ?>" role="dialog">
													...
													<!-- tampilkan data yang akan diubah kedalam form  -->
														<?php
															$id = $h['id'] ;
															$kamarubah = tampil("SELECT * FROM kamar WHERE id = $id ") ;
															$kamarubah = $kamarubah[0] ;
														?>
													<!-- end tampil data -->
													...
													<div class="modal-dialog" role="document">
			  											<div class="modal-content">
															...
															<form action="" method="post" enctype="multipart/form-data">
																	<input type="hidden" name="fotolama" value="<?= $kamarubah['foto'] ; ?>">
																	<input type="hidden" name="id" value="<?= $kamarubah['id'] ; ?>">
															<div class="modal-body">
				  												<div class="form-group">
					  												<label for="idhotel">Id Hotel</label>
					  												<input type="number" class="form-control" name="idhotel" id="idhotel" value="<?= $kamarubah['idhotel'] ;?>">
				  												</div>

																  <div class="form-group">
  																		<label for="tipe">Tipe</label>
  																		<?php
																			$tipekamar = ['kamar standar', 'kamar superior', 'kamar deluxe', 'kamar keluarga'];
  																		?>
  																		<select class="form-control" id="tipe" name="tipe">
    																	<?php
    																		foreach ($tipekamar as $option) {
      																			if ($option == $kamarubah['tipe']) {
				  												...
				  												<div class="form-group">
					  												...
					  												<input type="number" class="form-control" id="harga" name="harga" value="<?= $kamarubah['harga'] ; ?>" required>
				 												</div>
																<div class="form-group">
  																	<label for="ketersediaan">Ketersediaan</label>
  																	<?php
																		$ketersediaankamar = ['tersedia','tidak tersedia'];
  																	?>
  																	<select class="form-control" id="ketersediaan" name="ketersediaan">
    																	<?php
    																		foreach ($ketersediaankamar as $optionn) {
      																			if ($optionn == $kamarubah['ketersediaan']) {
        																			echo "<option value='$optionn' selected>$optionn</option>";
      																			} else {
        																			echo "<option value='$optionn'>$optionn</option>";
      																			}
    																		}
																		?>
  																	</select>
				  												...
				  												<div class="form-group">
					  												...
					  												<input type="number" class="form-control" id="rating" name="rating" value="<?= $kamarubah['rating'] ; ?>" required>
				  												</div>
																<div class="form-group">
																	<label for="fasilitas">Fasilitas</label>
																	<input type="text" class="form-control" id="fasilitas" name="fasilitas" value="<?= $kamarubah['fasilitas'] ; ?>" required>
																</div>
																<div class="form-group">
																	<label for="kapasitas">Kapasitas</label>
																	<input type="number" class="form-control" name="kapasitas" id="kapasitas" value="<?= $kamarubah['kapasitas'] ; ?>">
																</div>
																<div class="form-group">
    							...
    							<div class="modal-content">
									...
									<form action="" method="post" enctype="multipart/form-data">	
      									<div class="modal-body">
        									<div class="form-group">
		    									<label for="idhotel">Id hotel</label>
												<input type="number" class="form-control" required name="idhotel" id="idhotel">
											</div>
        									<div class="form-group">
		    									<label for="tipe">Tipe</label>
												<select class="form-control" id="tipe" name="tipe">
													<option>kamar standar</option>
													<option>kamar superior</option>
													<option>kamar deluxe</option>
													<option>kamar keluarga</option>
												</select>
											</div>
											<div class="form-group">
		    									<label for="harga">Harga</label>
											...
											<div class="form-group">
		    									<label for="rating">Rating</label>
												<input type="number" class="form-control" placeholder="masukan rating dari 1-5... " required name="rating" id="rating">
											</div>
											<div class="form-group">
												<label for="fasilitas">fasilitas</label>
												<input type="text" class="form-control" required name="fasilitas" id="fasilitas">
											</div>
											<div class="form-group">
												<label for="kapasitas">kapasitas</label>
												<input type="number" class="form-control" required name="kapasitas" id="kapasitas">
											</div>
											<div class="form-group">
												<label for="foto">foto</label>
